...
select P.Ano, count(distinct CodCliente) as QtdDeClientes
...
select	Ano,
		count(distinct CodCliente) as QtdeClientesReais
from tbvendas_f
group by Ano
order by Ano
...
select Ano,
		(VALOR_Area_Comercial/VALOR_AreaTotal)*100 as [Area_Comercial%],
		(VALOR_Area_Hibrida/VALOR_AreaTotal)*100 as [Area_Hibrida%],
		(VALOR_Area_Industrial/VALOR_AreaTotal)*100 as [Area_Industrial%],
		(VALOR_Area_Residencial/VALOR_AreaTotal)*100 as [Area_Residencial%],
